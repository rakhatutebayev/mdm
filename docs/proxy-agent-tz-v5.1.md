...
* **Список профилей:** отдельная страница консоли показывает все загруженные **Device Profile** с колонками: `profile_id`, `profile_name`, `profile_vendor`, `profile_version`, число строк в `output_mapping`, число устройств с этим профилем, **статус готовности**.
* **Реквизиты (обязательные поля §2.4):** на странице профиля администратор задаёт/правит **`profile_name`**, **`profile_vendor`**, **`profile_version`**; **`profile_id`** остаётся стабильным идентификатором (slug после импорта).
* **Статус готовности к приёму данных (агрегированный):**
  * *Not ready* — пустой или битый `output_mapping`.
  * *Ready (unused)* — маппинг валиден, но ни одно устройство не назначено на профиль.
  * *In use* — есть устройства; при недавнем `last_seen` — признак активного опроса.
  * *SNMP check OK* — после ручного **Verify SNMP** успешно выполнен SNMP GET по первому **скалярному** OID из маппинга на первом назначенном устройстве (OID с LLD-макросами `{#…}` для пробы не используются).
  * *SNMP check failed* — последняя ручная проверка не получила ответ.
* Результат **Verify SNMP** и время проверки сохраняются локально (KV) и отображаются в списке и на карточке профиля. Это не заменяет непрерывный поллер, но даёт явное подтверждение «профиль + SNMP + устройство работают».
* **Просмотр `output_mapping`:** на карточке профиля показывается превью JSON (первые N строк) для сверки после импорта Zabbix.
