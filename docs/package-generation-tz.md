...
- отказаться от сборки `MSI` и `EXE` на production-сервере;
...
Поддержка `MSI` допускается только как публикация готового артефакта из release
manifest. Production не должен собирать `MSI` локально.

### 7.5. Ошибки генерации
...
3. Собирать installer `EXE` через Inno Setup.
4. По возможности собирать `MSI` через WiX.
5. Публиковать артефакты в GitHub Releases.
6. Обновлять `backend/package_builder/agent_releases.json`.
...
- не устанавливать `nsis`, `wixl`, `msitools` и аналогичные Windows build tools
  для runtime генерации пакета.
