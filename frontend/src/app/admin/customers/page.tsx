...
export default function CustomersPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (modalMode === 'add') {
        const newC = await createCustomer(form.name.trim(), form.slug.trim());
    ...
    <div className={styles.page}>
      ...
      <div className={styles.toolbar}>
        <div className={styles.toolbarLeft}>
          <button className={styles.addBtn} onClick={openAdd}>
            <span className={styles.plus}>+</span> Add Customer
          </button>
          <button className={styles.actionBtn} disabled={!singleSelected} onClick={openEdit} title="Edit">
          ...
          <button className={`${styles.actionBtn} ${styles.deleteBtn}`} disabled={selectedCount === 0} onClick={() => handleDelete([...selected])} title="Delete selected">
            <svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor">
              <path d="M6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7H6v12zM19 4h-3.5l-1-1h-5l-1 1H5v2h14V4h-3.5z"/>
            </svg>
            Delete
          </button>
        </div>
      ...
      <div className={styles.tableWrap}>
        <table className={styles.table}>
          <thead>
            <tr>
              <th className={styles.thCheck}><input type="checkbox" checked={selected.size === filtered.length && filtered.length > 0} onChange={toggleAll} /></th>
          ...
          <tbody>
            {loading ? (
              ...
              <tr key={c.id} className={`${styles.tr} ${selected.has(c.id) ? styles.trSelected : ''}`} onClick={() => toggleRow(c.id)}>
                ...
                <td className={styles.td} onClick={(e) => e.stopPropagation()}>
                  <div className={styles.rowActions}>
                    <button className={styles.rowBtn} title="Edit" onClick={() => { setSelected(new Set([c.id])); setTimeout(() => { setForm({ name: c.name, slug: c.slug }); setEditId(c.id); setModalMode('edit'); }, 0); }}>
                      <svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 5.63l-2.34-2.34a1 1 0 00-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83a1 1 0 000-1.41z"/></svg>
                    </button>
                    <button className={`${styles.rowBtn} ${styles.rowBtnDanger}`} title="Delete" onClick={() => handleDelete([c.id])}>
      ...
      <div className={styles.pagination}>
        ...
        <div className={styles.paginationRight}>
          ...
          <button className={styles.pageBtn} disabled><svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor"><path d="M15.41 16.59L10.83 12l4.58-4.59L14 6l-6 6 6 6z"/></svg></button>
          <button className={styles.pageBtn} disabled={filtered.length <= rowsPerPage}><svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor"><path d="M8.59 16.59L13.17 12 8.59 7.41 10 6l6 6-6 6z"/></svg></button>
        </div>
      </div>
          ...
          <div className={styles.modal}>
            ...
            <div className={styles.modalBody}>
              <div className={styles.formRow}>
                ...
                <input type="text" className={styles.formInput} placeholder="e.g. NOCKO IT" value={form.name} autoFocus
                  onChange={(e) => {
              ...
              <div className={styles.formRow}>
                ...
                <input type="text" className={styles.formInput} placeholder="e.g. nocko-it" value={form.slug}
