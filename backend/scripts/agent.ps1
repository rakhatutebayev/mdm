...
# -- Command handlers --------------------------------------------------------
function Invoke-Lock         { rundll32.exe user32.dll,LockWorkStation }
function Invoke-Shutdown     { Stop-Computer -Force }
function Invoke-Restart      { Restart-Computer -Force }

function Invoke-ShowMessage {
    param($p)
...
function Invoke-InstallApp {
    param($p)
    if ($p.winget_id) {
        $wargs = @("install", "--id", $p.winget_id, "--silent", "--accept-source-agreements", "--accept-package-agreements")
        $r = & winget @wargs
        return ($r | Out-String)
    } elseif ($p.msi_url) {
    ...
    throw "No valid install source (winget_id / msi_url / msix_url required)"
}
...
function Invoke-CollectInventory { return (Get-HardwareInfo | ConvertTo-Json -Depth 5) }

function Start-MDMCommand {
    param($command)
    ...
    try {
        switch ($type) {
            "LOCK"              { Invoke-Lock }
            "LOCK_DEVICE"       { Invoke-Lock }
            "SHUTDOWN"          { Invoke-Shutdown }
            "REBOOT"            { Invoke-Restart }
            "RESTART"           { Invoke-Restart }
            "MESSAGE"           { Invoke-ShowMessage $payload }
            "RUN_SCRIPT"        { $output = Invoke-RunScript $payload }
            "INSTALL_APP"       { $output = Invoke-InstallApp $payload }
            "ANDROID_INSTALL_APP" { $output = Invoke-InstallApp $payload }
            "UNINSTALL_APP"     { Invoke-UninstallApp $payload }
            "SET_WALLPAPER"     { $output = Invoke-SetWallpaper $payload }
            "COLLECT_INVENTORY" { $output = Invoke-CollectInventory }
...
function Remove-MDMAgent {
    Unregister-ScheduledTask -TaskName $TASK_NAME -Confirm:$false -EA SilentlyContinue
    Write-Host "[OK] NOCKO MDM Agent removed." -ForegroundColor Yellow
}
...
# -- Entry point -------------------------------------------------------------
if     ($Install)   { Install-MDMAgent }
elseif ($Uninstall) { Remove-MDMAgent }
elseif ($CheckIn)   { Start-CheckIn }
elseif ($Status)    { Show-AgentStatus }
else {
    Write-Host ""
    ...
    Write-Host "  -Status      Show task status and last log lines"
