"""NOCKO Agent — Backup Page (Stub UI)"""
...
class BackupPage(QWidget):
    def __init__(self, backup_module=None, parent=None):
        ...
        desc = QLabel(
            "The Backup module will provide encrypted file-sync to NOCKO servers,\n"
            "scheduled backups, version history, and bandwidth throttling.\n\n"
            "Stay tuned for the next release!"
        )
        desc.setWordWrap(True)
        desc.setAlignment(Qt.AlignmentFlag.AlignCenter)
        desc.setStyleSheet(f"color: {BRAND['muted']}; font-size: 13px; line-height: 1.6;")

        roadmap = QLabel("📋  Roadmap: File sync engine → Scheduling → SFTP/S3 targets → Versioning")
